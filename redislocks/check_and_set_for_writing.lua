...
if not preservation_time or preservation_time < redis_time then
    redis.call('SET', check_write_target_preservation, redis_time, 'PXAT', tonumber(redis_time));
end
...
if not read_lock_expiring_time_in_microsecond then
    if redis.call('SET', check_write_target, redis_time, 'PXAT', tonumber(redis_time), 'NX') then
        return redis_time;
    end
    return '1'
end
